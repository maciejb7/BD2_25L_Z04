...
function ProfileCard({
  ...
  const [randomMusic, setRandomMusic] = useState<UserMusicFavorite | null>(
    null,
  );
  ...
  useEffect(() => {
    const loadUserData = async () => {
      ...
      try {
        ...
        const userMusicPromise = getUserFavoriteMusic(user.userId).catch(
          () => ({ favorites: [], count: 0 }),
        );
        ...
        if (userMusic.favorites && userMusic.favorites.length > 0) {
          const randomIndex = Math.floor(
            Math.random() * userMusic.favorites.length,
          );
  ...
  const findSharedHobby = (
    currentUserHobbies: UserHobby[],
    targetUserHobbies: UserHobby[],
  ): string | null => {
    ...
    for (const currentHobby of currentUserHobbies) {
      for (const targetHobby of targetUserHobbies) {
        if (
          currentHobby.hobby.hobby_category_id ===
          targetHobby.hobby.hobby_category_id
        ) {
          ...
          <div className="bg-purple-50 rounded-lg p-3">
            <div className="flex items-center space-x-3">
              <div className="relative">
                ...
                >
                  <i
                    className={`fas ${isPlaying ? "fa-pause" : "fa-play"} text-sm`}
                  ></i>
                </button>
              </div>
