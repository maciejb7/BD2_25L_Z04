...
/**
 * Service for handling music-related operations with Deezer API.
 */
export class MusicService {
  private static DEEZER_API_BASE = "https://api.deezer.com";
   ...
   */
  static async getAlbumDetails(albumId: number): Promise<DeezerAlbumDetails> {
    try {
      const response = await axios.get<DeezerAlbumDetails>(
        `${this.DEEZER_API_BASE}/album/${albumId}`,
      );
  ...
  static async saveTrack(
    ...
    try {
      ...
      // Get album details
      const albumDetails = await this.getAlbumDetails(track.album.id);
   ...
   */
  static async removeFavoriteTrack(
    userId: string,
    trackId: number,
  ): Promise<boolean> {
    try {
   ...
   */
  static async getTrackDetails(trackId: number): Promise<{
    track: MusicTrack;
    artist: MusicArtist;
    album: MusicAlbum;
  } | null> {
