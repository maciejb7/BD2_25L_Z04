...
export class LocationController {
     ...
     * @param res Response
     */
    static async getCurrentUserLocation(req: Request, res: Response): Promise<void> {
        try {
            const { userId } = req.user as { userId: string };
    ...
    static async getUserLocation(req: Request, res: Response): Promise<void> {
        try {
            ...
            res.status(500).json({
                message: "Wystąpił błąd podczas pobierania lokalizacji innego użytkownika",
            });
        }
    }
     ...
     * @param res Response
     */
    static async getDistanceToUser(req: Request, res: Response): Promise<void> {
        try {
